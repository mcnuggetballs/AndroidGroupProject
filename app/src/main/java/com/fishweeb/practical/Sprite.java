...
public class Sprite
{
    ...
    public void ContinueAnimationFrames(int _start , int _end)
    {
        int currentRow = (_end/col) - 1;
        currentFrame = (currentFrame%col) + (currentRow*col);
        startFrame = _start;
        endFrame = _end;
    }

    public int GetCurrentFrameColumn()
    {
        int currentRow = (currentFrame/col);
        int currentCol = currentFrame - currentRow*col;
        return currentCol;
    }
